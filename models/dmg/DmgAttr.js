...
let DmgAttr = {
  ...
  getAttr ({ originalAttr, attr, weapon, char, game = 'gs' }) {
    ...
    // 技能属性记录
    lodash.forEach((game === 'gs' ? 'a,a2,a3,e,q,nightsoul' : 'a,a2,a3,e,e1,e2,q,q2,t,t2,z,dot,break').split(','), (key) => {
      ret[key] = ret[key] || {
        pct: 0, // 倍率加成
  ...
  calcAttr ({ originalAttr, buffs, meta, artis, params = {}, incAttr = '', reduceAttr = '', talent = '', game = 'gs' }) {
    ...
    lodash.forEach(buffs, (buff) => {
      ...
      lodash.forEach(buff.data, (val, key) => {
        ...
        // 技能提高
        let tRet = /^(a|a2|a3|e|e1|e2|z|q|q2|t|t2|dot|break|nightsoul)(Def|Ignore|Dmg|Enemydmg|Plus|Pct|Cpct|Cdmg|Multi|Elevated)$/.exec(key)
        if (tRet) {
          attr[tRet[1]][tRet[2].toLowerCase()] += val * 1 || 0
          return
        }

        let aRet = /^(truedmg|mastery|cpct|cdmg|heal|recharge|dmg|enemydmg|phy|shield|speed|stance)(Plus|Pct|Inc)?$/.exec(key)
        if (aRet) {
